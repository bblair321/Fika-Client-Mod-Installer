...
const backupOverwritten = '{{BACKUP_OVERWRITTEN}}' === 'true';
...
function backupExistingFiles(tempArchive, extractDir) {
  // Copies files that the archive would overwrite into a timestamped
  // backup folder inside the install directory, preserving relative paths.
  // Returns { backupPath, count } or null when nothing needed backing up.
...
async function extractFiles(extractDir) {
  try {
    ...
    // Back up files that would be overwritten (opt-in at packaging time)
    let backupInfo = null;
    if (backupOverwritten) {
      try {
        backupInfo = backupExistingFiles(tempArchive, extractDir);
      } catch (backupError) {
        // A failed backup must not corrupt the install — abort before overwriting
        try { fs.unlinkSync(tempArchive); } catch (e) {}
        return {
          success: false,
          error: 'Could not back up existing files: ' + backupError.message +
            '. Nothing was installed or overwritten.'
        };
      }
    }
    ...
    return {
      ...
      backupCount: backupInfo ? backupInfo.count : 0
    };
  } catch (error) {
...
'body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif; margin: 0; padding: 0; display: block; overflow: hidden; width: 900px; height: 700px; background: #1a2332 !important; }' + String.fromCharCode(10) +
'.container { background: #1a2332 !important; -webkit-border-radius: 16px; -moz-border-radius: 16px; -ms-border-radius: 16px; border-radius: 16px; box-shadow: 0 10px 40px rgba(0,0,0,0.3); width: 900px !important; height: 700px !important; overflow: hidden; animation: slideIn 0.3s ease-out; border: none; display: flex; flex-direction: column; position: relative; margin: 0; padding: 0; }' + String.fromCharCode(10) +
...
'.step h2 { font-size: 1.5em; margin-bottom: 20px; color: #333; border-bottom: 2px solid #b45309; padding-bottom: 10px; font-weight: 600; border-radius: 4px; }' + String.fromCharCode(10) +
'.step p { margin-bottom: 20px; color: #666; line-height: 1.6; font-size: 1em; }' + String.fromCharCode(10) +
'.info-box { background: #f8f9fa; border: 1px solid #e0e0e0; padding: 20px; -webkit-border-radius: 6px; -moz-border-radius: 6px; -ms-border-radius: 6px; border-radius: 6px; margin: 20px 0; }' + String.fromCharCode(10) +
...
'.path-input { flex: 1; padding: 12px 16px; border: 2px solid #e0e0e0; -webkit-border-radius: 6px; -moz-border-radius: 6px; -ms-border-radius: 6px; border-radius: 6px; font-size: 1em; background: white; color: #333; transition: border-color 0.3s; font-family: "Segoe UI", sans-serif; }' + String.fromCharCode(10) +
'.path-input:focus { outline: none; border-color: #b45309; }' + String.fromCharCode(10) +
'.path-input[readonly] { background-color: #f5f5f5; cursor: pointer; }' + String.fromCharCode(10) +
'.btn { padding: 12px 24px; border: none; -webkit-border-radius: 6px; -moz-border-radius: 6px; -ms-border-radius: 6px; border-radius: 6px; font-size: 1em; font-weight: 600; cursor: pointer; transition: all 0.3s; white-space: nowrap; }' + String.fromCharCode(10) +
'.btn-primary { background: #b45309; color: white; }' + String.fromCharCode(10) +
'.btn-primary:hover:not(:disabled) { background: #92400e; }' + String.fromCharCode(10) +
...
'.progress-fill { height: 100%; background: #b45309; width: 0%; transition: width 0.3s; border-radius: 15px; }' + String.fromCharCode(10) +
...
'.install-path { background: #f8f9fa; padding: 15px; -webkit-border-radius: 6px; -moz-border-radius: 6px; -ms-border-radius: 6px; border-radius: 6px; font-family: "Consolas", "Monaco", "Courier New", monospace; word-break: break-all; margin: 20px 0; border: 1px solid #e0e0e0; color: #333; font-size: 0.9em; }' + String.fromCharCode(10) +
...
'.step-dot.active { background: #b45309; width: 30px; border-radius: 5px; }' + String.fromCharCode(10) +
...
'<div id="welcome-step" class="step active"><h2>Welcome</h2><p>Thank you for choosing ' + escapeHtml(appName || "this package") + '. This wizard will guide you through the installation process.</p><div class="info-box" style="-webkit-border-radius: 6px; -moz-border-radius: 6px; -ms-border-radius: 6px; border-radius: 6px;"><strong>Application:</strong> ' + escapeHtml(appName || "Package") + '<br><strong>Package Size:</strong> <span id="packageSize">' + formattedSize + '</span></div><div style="text-align: center; margin-top: 30px;"><button class="btn btn-primary" id="welcomeNextBtn" onclick="nextStep()" style="cursor: pointer; opacity: 1; min-width: 150px; -webkit-border-radius: 6px; -moz-border-radius: 6px; -ms-border-radius: 6px; border-radius: 6px;">Continue</button></div></div>' + String.fromCharCode(10) +
'<div id="directory-step" class="step"><h2>Choose Installation Location</h2><p>Please select the destination folder where you would like to install the files.</p><div class="input-group"><input type="text" id="installPath" class="path-input" placeholder="Click Browse to select a folder..." readonly style="-webkit-border-radius: 6px; -moz-border-radius: 6px; -ms-border-radius: 6px; border-radius: 6px;"><button class="btn btn-primary" id="browseBtn" onclick="selectFolder()" style="-webkit-border-radius: 6px; -moz-border-radius: 6px; -ms-border-radius: 6px; border-radius: 6px;">Browse</button></div><div class="button-group"><button class="btn btn-secondary" id="dirBackBtn" onclick="prevStep()" style="-webkit-border-radius: 6px; -moz-border-radius: 6px; -ms-border-radius: 6px; border-radius: 6px;">Back</button><button class="btn btn-primary" id="nextBtn" onclick="nextStep()" disabled style="-webkit-border-radius: 6px; -moz-border-radius: 6px; -ms-border-radius: 6px; border-radius: 6px;">Install</button></div></div>' + String.fromCharCode(10) +
...
'<div id="complete-step" class="step"><h2>Installation Complete</h2><div class="success-icon">✓</div><p style="text-align: center; margin-bottom: 20px; font-size: 1em; color: #333;">All files have been successfully installed to:</p><p class="install-path" id="finalPath" style="-webkit-border-radius: 6px; -moz-border-radius: 6px; -ms-border-radius: 6px; border-radius: 6px;"></p><p id="backupNote" style="display: none; text-align: center; color: #555; font-size: 0.85em; margin: 12px 0; padding: 10px; background: #f8f9fa; border: 1px solid #e0e0e0; -webkit-border-radius: 6px; -moz-border-radius: 6px; -ms-border-radius: 6px; border-radius: 6px; word-break: break-all;"></p><div class="button-group" style="justify-content: center; margin-top: 30px;"><button class="btn btn-success" onclick="openFolder()" style="-webkit-border-radius: 6px; -moz-border-radius: 6px; -ms-border-radius: 6px; border-radius: 6px;">Open Folder</button><button class="btn btn-primary" onclick="closeInstaller()" style="-webkit-border-radius: 6px; -moz-border-radius: 6px; -ms-border-radius: 6px; border-radius: 6px;">Finish</button></div></div>' + String.fromCharCode(10) +
'<div id="error-step" class="step"><h2>Installation Failed</h2><div class="error-icon">✗</div><p id="errorMessage" style="text-align: center; color: #721c24; font-weight: 400; margin: 20px 0; padding: 15px; background: #f8d7da; -webkit-border-radius: 6px; -moz-border-radius: 6px; -ms-border-radius: 6px; border-radius: 6px; border: 1px solid #f5c6cb; font-size: 0.9em;"></p><div style="text-align: center; margin-top: 30px;"><button class="btn btn-primary" onclick="showStep(1)" style="-webkit-border-radius: 6px; -moz-border-radius: 6px; -ms-border-radius: 6px; border-radius: 6px;">Try Again</button></div></div>' + String.fromCharCode(10) +
...
'    var centerY = (screen.availHeight / 2) - 350;' + String.fromCharCode(10) +
'    window.moveTo(centerX, centerY);' + String.fromCharCode(10) +
'    document.body.style.background = "#1a2332";' + String.fromCharCode(10) +
'    var container = document.querySelector(".container");' + String.fromCharCode(10) +
'    if (container) container.style.background = "#1a2332";' + String.fromCharCode(10) +
'    applyDefaultInstallPath();' + String.fromCharCode(10) +
'  } catch (e) {}' + String.fromCharCode(10) +
'};' + String.fromCharCode(10) +
'function applyDefaultInstallPath() {' + String.fromCharCode(10) +
'  try {' + String.fromCharCode(10) +
'    if (!defaultExtractPath || defaultExtractPath === "{{" + "DEFAULT_EXTRACT_PATH" + "}}") {' + String.fromCharCode(10) +
'      return;' + String.fromCharCode(10) +
'    }' + String.fromCharCode(10) +
'    selectedPath = defaultExtractPath;' + String.fromCharCode(10) +
...
'var defaultExtractPath = "{{DEFAULT_EXTRACT_PATH}}";' + String.fromCharCode(10) +
...
'          var backupInfo = parts.length > 3 ? parts[3] : "";' + String.fromCharCode(10) +
...
'            var backupEl = document.getElementById("backupNote");' + String.fromCharCode(10) +
'            if (backupEl && backupInfo) {' + String.fromCharCode(10) +
'              backupEl.innerText = backupInfo;' + String.fromCharCode(10) +
'              backupEl.style.display = "block";' + String.fromCharCode(10) +
'            }' + String.fromCharCode(10) +
          ...
          if (content.indexOf('EXTRACT|') === 0) {
            ...
            if (parts.length >= 2) {
              ...
              extractFiles(extractPath).then(result => {
                if (result.success) {
                  let message = 'COMPLETE|' + result.extractionPath + '|SUCCESS';
                  if (result.backupPath && result.backupCount > 0) {
                    message += '|' + result.backupCount + ' existing file(s) backed up to: ' + result.backupPath;
                  }
                  fs.writeFileSync(commFile, message, 'utf8');
                } else {
